...
class SaleOrder(models.Model):
    ...
    def export_quotation_to_excel(self):
        ...
        for line in self.order_line:
            authors = (
                line.product_id.author_ids
                if line.product_id and line.product_id.author_ids
                else []
            )
            editor = (
                line.product_id.editor_id.name
                if line.product_id and line.product_id.editor_id
                else ""
            )
            ...
            worksheet.write(
                row,
                2,
                " ".join([author.name for author in authors]) or "",
                normal_format,
            )  # Assuming you have author field
            worksheet.write(
                row, 3, editor or "", normal_format
            )  # Assuming you have publisher field
            worksheet.write(
                row, 4, line.product_id.categ_id.name or "", normal_format
            )  # Assuming category field
            worksheet.write(
                row, 5, line.product_id.categ_id.parent_id.name or "", normal_format
            )  # Assuming subcategory field
            worksheet.write(
                row, 6, line.product_id.release_year or "", normal_format
            )  # Assuming publication year field
